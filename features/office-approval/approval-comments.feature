...
Feature: OfficeApprovalComments
  ...
  Scenario: Verify an Office Approval Authority can see the Comments block on the View Permit page - UI (5308)
    Given I submit permit submit_underwater_simultaneous via service with 9015 user and set to pending office approval state and env sit
  ...
  Scenario: Verify the Designation list contains all necessary roles (5310)
    Given I submit permit submit_non_intrinsical_camera via service with 9015 user and set to pending office approval state and env sit
    And I navigate to OA link
    And I click on Add Comments button
