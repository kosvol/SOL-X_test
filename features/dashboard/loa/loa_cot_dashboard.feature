@loa_cot_dashboard
Feature: LOA Dashboard for COT Vessel(At present can be used for LNG)

  #TODO: add missing tests, move existing tests from other feature

  Scenario Outline: Verify Master is able to View pin (SOL-3373)
  ...
  Scenario: Verify user with non-Master is not able to View pin (SOL-3373)
    Given Dashboard open dashboard page
    And Dashboard verify the local time
    Then Dashboard open hamburger menu
    And NavigationDrawer navigate to Dashboard "Crew Management"
  ...
  Scenario Outline: Verify a time zone changer is able to update ship time (SOL-8314).
  ...
  Scenario: Verify a not time zone changer is not able to update ship time (SOL-8314).
    And Dashboard open dashboard page
    And Dashboard click time button
