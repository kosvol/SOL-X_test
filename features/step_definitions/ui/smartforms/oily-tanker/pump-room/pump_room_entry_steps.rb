...
Then(/^I should see entry log details display as (filled|filled api)$/) do |condition|
  is_equal(on(PumpRoomEntry).entry_log_table_elements.first.text, "A/M #{EnvironmentSelector.vessel_type} A/M")
  ...
  is_equal(on(PumpRoomEntry).entry_log_table_elements[10].text, "C/O #{EnvironmentSelector.vessel_type} C/O")
end
