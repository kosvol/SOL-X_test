...
Then(/^I should see section (.*) screen$/) do |which_section|
  ...
  when '1'
    is_equal(on(NavigationPage).section_header_elements.first.text, 'Section 1: Task Description')
  when '2'
    is_equal(on(NavigationPage).section_header_elements.first.text, 'Section 2: Approving Authority')
  when '8'
    is_equal(on(NavigationPage).section_header_elements.first.text, 'Section 8: Task Status & EIC Normalisation')
  when '6'
    is_equal(on(NavigationPage).section_header_elements.first.text, 'Section 6: Gas Testing/Equipment')
  else
