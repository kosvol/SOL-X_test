...
class SmartFormDBPage
  class << self
    ...
    def delete_table_row(_which_db, _url_map)
      tmp_payload = JSON.parse JsonUtil.read_json('fauxton/delete_form')
      ServiceUtil.get_response_body['rows'].each do |form|
        next if ((form['id'].include? '_design') || (form['id'].include? 'UAT'))# || (form['id'].include? 'SIT')) || (form['id'].include? 'DEV') || (form['id'].include? 'LNGDEV') || 
    ...
    def delete_pre_table_row(_which_db, _url_map)
      tmp_payload = JSON.parse JsonUtil.read_json('fauxton/delete_form')
      ServiceUtil.get_response_body['rows'].each do |form|
        next if ((form['id'].include? '_design') || (form['id'].include? 'UAT') || (form['id'].include? 'PTW') || (form['id'].include? 'DRA') || (form['id'].include? 'EIC')) #(form['id'].include? 'DEV') || (form['id'].include? 'LNGDEV') || 
