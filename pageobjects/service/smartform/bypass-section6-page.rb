...
class BypassPage
  ...
  def trigger_forms_submission(_user)
    ### init ptw form
    ...
    set_selected_level2_permit(ServiceUtil.get_response_body['data']['createForm']['_id'])

    ### add time offset to ptw
    add_time_offset_to_ptw = JSON.parse JsonUtil.read_json('ptw/1.date_with_offset')
    add_time_offset_to_ptw['variables']['id'] = get_selected_level2_permit
    ...
    init_dra['variables']['parentFormId'] = get_selected_level2_permit
    ...
    section2['variables']['formId'] = get_selected_level2_permit
    section2['variables']['answers'].last['value'] = "{\"dateTime\":\"#{get_current_date_time}\",\"utcOffset\":#{get_offset}}"
    JsonUtil.create_request_file('ptw/2.mod_save_section0_details', section2)
    ServiceUtil.post_graph_ql('ptw/2.mod_save_section0_details', _user)
  ...
  def save_different_form_section(_which_json, _user)
    ...
    section['variables']['formId'] = get_selected_level2_permit
