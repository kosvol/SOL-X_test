...
class WorkResetrHoursPage
  class << self
    def load_work_rest_hour
      ...
      tmp_payload['docs'].each_with_index do |doc, index|
        userid = doc['userId']
        userid[0, 4] = 'SIT' if $current_environment == 'sit'
        tmp_payload['docs'][index]['userId'] = userid
      end

      # ## 13h59m SOLX0004
      ttt = tmp_payload['docs'][0]['startTime']
      ttt[0, 24] =
        ServiceUtil.craft_date_time_format(DateTime.now.new_offset(Rational(-7, 24)).strftime('%Y').to_i,
                                           DateTime.now.new_offset(Rational(-7, 24)).strftime('%m').to_i,
                                           DateTime.now.new_offset(Rational(-7, 24)).strftime('%d').to_i,
                                           DateTime.now.new_offset(Rational(-7, 24)).strftime('%H').to_i, 0, 0)
      tmp_payload['docs'][0]['startTime'] = ttt
      ttt = tmp_payload['docs'][0]['endTime']
      ttt[0, 24] =
        ServiceUtil.craft_date_time_format(DateTime.now.new_offset(Rational(0, 24)).strftime('%Y').to_i,
                                           DateTime.now.new_offset(Rational(0, 24)).strftime('%m').to_i,
                                           DateTime.now.new_offset(Rational(0, 24)).strftime('%d').to_i,
                                           DateTime.now.new_offset(Rational(0, 24)).strftime('%H').to_i, 0, 0)
      tmp_payload['docs'][0]['endTime'] = ttt

      ttt = tmp_payload['docs'][1]['startTime']
      ttt[0, 24] =
        ServiceUtil.craft_date_time_format(DateTime.now.new_offset(Rational(-10, 24)).strftime('%Y').to_i,
                                           DateTime.now.new_offset(Rational(-10, 24)).strftime('%m').to_i,
                                           DateTime.now.new_offset(Rational(-10, 24)).strftime('%d').to_i,
                                           DateTime.now.new_offset(Rational(-10, 24)).strftime('%H').to_i, 0, 0)
      ...
      ttt[0, 24] =
        ServiceUtil.craft_date_time_format(DateTime.now.new_offset(Rational(-13, 24)).strftime('%Y').to_i,
                                           DateTime.now.new_offset(Rational(-13, 24)).strftime('%m').to_i,
                                           DateTime.now.new_offset(Rational(-13, 24)).strftime('%d').to_i,
                                           DateTime.now.new_offset(Rational(-13, 24)).strftime('%H').to_i, 0, 0)
      ...
      # ttt[0,10] = Date.today.strftime("%Y-%m-%d")
      ttt[0, 24] =
        ServiceUtil.craft_date_time_format(DateTime.now.new_offset(Rational(-12, 24)).strftime('%Y').to_i,
                                           DateTime.now.new_offset(Rational(-12, 24)).strftime('%m').to_i,
                                           DateTime.now.new_offset(Rational(-12, 24)).strftime('%d').to_i,
                                           DateTime.now.new_offset(Rational(-12, 24)).strftime('%H').to_i, 0, 0)
      ...
      ttt[0, 24] =
        ServiceUtil.craft_date_time_format(DateTime.now.new_offset(Rational(-3, 24)).strftime('%Y').to_i,
                                           DateTime.now.new_offset(Rational(-3, 24)).strftime('%m').to_i,
                                           DateTime.now.new_offset(Rational(-3, 24)).strftime('%d').to_i,
                                           DateTime.now.new_offset(Rational(-3, 24)).strftime('%H').to_i, 0, 0)
      # ttt[0,10] = Date.today.strftime("%Y-%m-%d")
      ...
      ttt[0, 24] =
        ServiceUtil.craft_date_time_format(DateTime.now.new_offset(Rational(-6, 24)).strftime('%Y').to_i,
                                           DateTime.now.new_offset(Rational(-6, 24)).strftime('%m').to_i,
                                           DateTime.now.new_offset(Rational(-6, 24)).strftime('%d').to_i,
                                           DateTime.now.new_offset(Rational(-6, 24)).strftime('%H').to_i, 0, 0)
      # ttt[0,10] = Date.today.strftime("%Y-%m-%d")
      ...
      ttt[0, 24] =
        ServiceUtil.craft_date_time_format(DateTime.now.new_offset(Rational(-4, 24)).strftime('%Y').to_i,
                                           DateTime.now.new_offset(Rational(-4, 24)).strftime('%m').to_i,
                                           DateTime.now.new_offset(Rational(-4, 24)).strftime('%d').to_i,
                                           DateTime.now.new_offset(Rational(-4, 24)).strftime('%H').to_i, 0, 0)
      # ttt[0,10] = Date.today.strftime("%Y-%m-%d")
      ...
      ttt[0, 24] =
        ServiceUtil.craft_date_time_format(DateTime.now.new_offset(Rational(-8, 24)).strftime('%Y').to_i,
                                           DateTime.now.new_offset(Rational(-8, 24)).strftime('%m').to_i,
                                           DateTime.now.new_offset(Rational(-8, 24)).strftime('%d').to_i,
                                           DateTime.now.new_offset(Rational(-8, 24)).strftime('%H').to_i, 0, 0)
      # ttt[0,10] = Date.today.strftime("%Y-%m-%d")
      ...
      ttt[0, 24] =
        ServiceUtil.craft_date_time_format(DateTime.now.new_offset(Rational(-11, 24)).strftime('%Y').to_i,
                                           DateTime.now.new_offset(Rational(-11, 24)).strftime('%m').to_i,
                                           DateTime.now.new_offset(Rational(-11, 24)).strftime('%d').to_i,
                                           DateTime.now.new_offset(Rational(-11, 24)).strftime('%H').to_i, 0, 0)
      # ttt[0,10] = Date.today.strftime("%Y-%m-%d")
      ...
      ttt[0, 24] =
        ServiceUtil.craft_date_time_format(DateTime.now.new_offset(Rational(-2, 24)).strftime('%Y').to_i,
                                           DateTime.now.new_offset(Rational(-2, 24)).strftime('%m').to_i,
                                           DateTime.now.new_offset(Rational(-2, 24)).strftime('%d').to_i,
                                           DateTime.now.new_offset(Rational(-2, 24)).strftime('%H').to_i, 0, 0)
      # ttt[0,10] = Date.today.strftime("%Y-%m-%d")
      ...
      ttt[0, 24] =
        ServiceUtil.craft_date_time_format(DateTime.now.new_offset(Rational(-5, 24)).strftime('%Y').to_i,
                                           DateTime.now.new_offset(Rational(-5, 24)).strftime('%m').to_i,
                                           DateTime.now.new_offset(Rational(-5, 24)).strftime('%d').to_i,
                                           DateTime.now.new_offset(Rational(-5, 24)).strftime('%H').to_i, 0, 0)
      # ttt[0,10] = Date.today.strftime("%Y-%m-%d")
      ...
      ttt[0, 24] =
        ServiceUtil.craft_date_time_format(DateTime.now.new_offset(Rational(-9, 24)).strftime('%Y').to_i,
                                           DateTime.now.new_offset(Rational(-9, 24)).strftime('%m').to_i,
                                           DateTime.now.new_offset(Rational(-9, 24)).strftime('%d').to_i,
                                           DateTime.now.new_offset(Rational(-9, 24)).strftime('%H').to_i, 0, 0)
      # ttt[0,10] = (Date.today+1).strftime("%Y-%m-%d")
