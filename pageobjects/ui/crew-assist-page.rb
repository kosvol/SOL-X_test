...
class CrewAssistPage < DashboardPage
  ...
  def is_crew_assist_dialog_details
    ...
    crew_assist_details_elements.each do |_detail|
      list_of_dialog_details << _detail.text
    end
  ...
  def is_crew_location_indicator_green
    $browser.find_element(:xpath, @@crew_assist_location_indicator.to_s).css_value('background-color').to_s === 'rgba(216, 75, 75, 1)'
  end
