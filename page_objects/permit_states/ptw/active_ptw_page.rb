...
class ActivePTWPage < BasePermitStatesPage
  ...
  ACTIVE_PTW = {
    ...
    view_btn: "//*[span='%s']/*[@class='note-row']/button//span[(text()='View')]",
  ...
  def verify_view_termination_btns(permit_id)
    wait_for_permit_display("//ul[starts-with(@class,'FormsList__Container')]/li/span[text()='#{permit_id}])']")
    find_element(ACTIVE_PTW[:view_terminate_btn] % permit_id)
    find_element(ACTIVE_PTW[:view_btn] % permit_id)
    verify_element_not_exist(ACTIVE_PTW[:gas_test_btn] % permit_id)
  end
end
