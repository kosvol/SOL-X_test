...
Then(/^I (should|should not) see gas reader sections$/) do |condition|
  sleep 1
  is_true(on(Section6Page).gas_reader_section?) if condition == 'should'
  is_true(!on(Section6Page).gas_reader_section?) if condition == 'should not'
end
...
And(/^I should not see gas reader dialog box$/) do
  raise 'Gas reader dialog box enabled' if on(CommonFormsPage).done_btn_elements.any?
end
