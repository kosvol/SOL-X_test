...
Then (/^I should see (.+) as button text$/) do |update_or_view|
  update_reading_or_view_btn = on(CreatedPermitToWorkPage).select_created_permit_with_param(on(BypassPage).get_permit_id)
...
And (/^I (.+) permit with (.+) rank and (.+) pin$/) do |_update_or_terminate, _rank, _pin|
  ...
  if _update_or_terminate === 'update active'
    ...
    termination_btn = on(ActiveStatePage).get_termination_btn(on(BypassPage).get_permit_id)
